...
function Step3Report({ report }) {
  ...
  const answersForATS =
    Array.isArray(questionWiseScore) && questionWiseScore.some((q) => q?.answer)
      ? questionWiseScore.map((q) => q?.answer || "")
      : report?.__answers || [];
  const atsResult = computeATSScore(answersForATS);
  ...
  const atsStartY = (doc.lastAutoTable && doc.lastAutoTable.finalY) || (currentY + 10);
  ...
  doc.text("ATS Resume Compatibility Score", margin, y);

  y += 10;

  doc.setFontSize(22);
  doc.setTextColor(16, 185, 129);
  doc.text(`${atsResult.overall}%`, margin, y);

  doc.setFontSize(11);
  doc.setTextColor(55, 65, 81);
  doc.text(atsResult.label, margin + 35, y);
  ...
  const breakdownLines = [
    `Keyword Match Score: ${atsResult.breakdown.keywordMatchScore}%`,
    `Clarity Score: ${atsResult.breakdown.clarityScore}%`,
    `Structure Score: ${atsResult.breakdown.structureScore}%`,
  ];
  ...
  atsResult.suggestions.forEach((s) => {
    const wrapped = doc.splitTextToSize(`• ${s}`, maxWidth);
      ...
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        <div className='md:mb-10 w-full flex items-start gap-4 flex-wrap'>
          <button
            ...
            className='mt-1 p-3 rounded-full bg-white dark:bg-gray-900 shadow hover:shadow-md transition'><FaArrowLeft className='text-gray-600 dark:text-gray-300' /></button>

          <div>
            <h1 className='text-3xl font-bold flex-nowrap text-gray-800 dark:text-gray-100'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8'>
        ...
        <div className='space-y-6'>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-white dark:bg-gray-900 rounded-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8 text-center border border-transparent dark:border-gray-700">
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className='bg-white dark:bg-gray-900 rounded-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8 border border-transparent dark:border-gray-700'>
          ...
          <ATSScoreCard answers={answersForATS} />
        ...
        <div className='lg:col-span-2 space-y-6'>
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className='bg-white dark:bg-gray-900 rounded-2xl sm:rounded-3xl shadow-lg p-5 sm:p-8 border border-transparent dark:border-gray-700'>
            ...
            <div className='space-y-6'>
              {questionWiseScore.map((q, i) => (
                <div key={i} className='bg-gray-50 dark:bg-gray-800 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-gray-700'>
                  ...
                  <div className='bg-green-50 dark:bg-gray-900 border border-green-200 dark:border-gray-700 p-4 rounded-lg'>
